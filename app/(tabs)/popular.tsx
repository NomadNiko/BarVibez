...
export default function PopularScreen() {
  ...
  const handleSliderChange = useCallback(
    (value: number) => {
      ...
      if (filteredCocktails.length > 0) {
        const cardWidth = screenWidth * 0.8 + 16;
        const maxOffset = Math.max(0, (filteredCocktails.length - 1) * cardWidth);
        const targetOffset = (value / 100) * maxOffset;
  ...
  const handleSliderComplete = useCallback(
    (value: number) => {
      if (filteredCocktails.length > 0) {
        ...
        const nearestCardIndex = Math.round(targetOffset / cardWidth);
        const snappedOffset = nearestCardIndex * cardWidth;
        
        // Snap to the nearest card
        flatListRef.current?.scrollToOffset({ offset: snappedOffset, animated: true });
        
        // Update slider to match snapped position
        const snappedPercentage = (snappedOffset / maxOffset) * 100;
        setSliderValue(Math.min(100, Math.max(0, snappedPercentage)));
      }
    },
    [filteredCocktails.length]
  );
  ...
  const handleScroll = useCallback(
    (event: any) => {
      if (filteredCocktails.length > 1) {
        ...
        const percentage = (currentOffset / maxOffset) * 100;
  ...
  const renderCocktailItem = ({
    item,
    index,
  }: {
    item: Cocktail | UserCocktail;
    index: number;
  }) => (
